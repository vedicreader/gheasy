...
Docs: https://vedicreader.github.io/gheasy/workflow.html.md"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/01_workflow.ipynb.

# %% auto #0
__all__ = ['yaml_instance', 'WorkflowNode', 'StepBuilder', 'StrategyBuilder', 'JobBuilder', 'TriggerBuilder', 'WorkflowBuilder',
           'WorkflowDoc', 'Workflow', 'docker_build_push', 'pages_deploy', 'uv_ci']
...
def to_yaml(self:WorkflowDoc) -> str:
    import io
    buf = io.StringIO()
    yaml_instance().dump(self._rendered, buf)
    return buf.getvalue()

@patch
def save(self:WorkflowDoc, path:str|Path) -> Path:
    p = Path(path); p.parent.mkdir(parents=True, exist_ok=True)
    with open(p, "w") as f: yaml_instance().dump(self._rendered, f)
    return p
