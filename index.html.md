...
[`uv_ci`](https://vedicreader.github.io/gheasy/workflow.html#uv_ci)
creates a complete Python CI workflow:
...
accepts the repository root or any path inside it. `brief` returns a
compact record for prompts and status lines. `info` returns the
branches, remotes, changes, tags, stashes, active operation, filter
health, and recovery points.
...
`divergence` compares the current branch with its upstream. It rehearses
fast-forward, merge, rebase, and reset, then reports conflicts and a
recommendation. `merge_preview` and `rebase_preview` inspect a specific
integration without changing refs, the index, or the working tree.
...
Guarded mutations record a safepoint before they run. The result
includes an `undo` token. Pass that token to `undo` to restore the
earlier branch, commit, and uncommitted work.
...
`review` compares branch work from the merge base. `review_file` loads
one changed file on demand. `commit_review` and `commit_file` provide
the same view for one commit.
...
Use the workflow builder directly when a deployment needs custom steps.
This example tests every push and deploys
[`main`](https://vedicreader.github.io/gheasy/core.html#main) to Fly.io:
...
Projects can configure Git LFS and copy local environment values to
...
sends every value in `.env` as a secret:
...
Workflow steps can call code from another package. This example uses
`dockeasy` to generate a Dockerfile, then builds and publishes the image
